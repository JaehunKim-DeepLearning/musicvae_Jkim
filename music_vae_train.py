...
try:
  ...
  pass
LOG_FILENAME = './log/' + FLAGS.config
now = str(datetime.now())
f = open(LOG_FILENAME, "a")
f.write(now + '\tModel Train Start\n')
f.close()

class LoggingTensorHook2(tf.train.LoggingTensorHook):
  def _log_tensors(self, tensor_values):
    ...
    now = str(datetime.now())
    f = open(LOG_FILENAME, "a")
    print(now + '\t' + str(tensor_values), file=f)
    f.close()
...
def train(train_dir,
  ...
  with tf.Graph().as_default():
    with tf.device(tf.train.replica_device_setter(
      ...
      hooks.append(LoggingTensorHook2(logging_dict, every_n_iter=100))
      #hooks.append(LoggingTensorCallback(every_n_iter=100))
      if num_steps:
